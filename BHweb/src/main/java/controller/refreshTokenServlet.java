...
public class refreshTokenServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            // Ghi lại nhật ký ngay sau khi xác thực refresh token thành công.
            AuditLogs log = new AuditLogs(userId, "TOKEN_REFRESH", "Users", String.valueOf(userId));
            auditLogDAO.save(log);

            // Tạo một đối tượng User tạm thời để truyền vào hàm generateToken
            User userFromToken = new User();
            userFromToken.setId(userId);
            userFromToken.setUsername(username);
            userFromToken.setRole(role);

            String newAccessToken = JwtUtils.generateToken(
                userFromToken,
                JwtConfig.ACCESS_TOKEN_EXPIRATION_MS,
    ...
    private Claims validateAndGetClaims(String token) throws Exception {
        Jws<Claims> claimsJws = Jwts.parserBuilder()
                                    .setSigningKey(JwtConfig.JWT_REFRESH_SECRET_KEY)
                                    .build()
